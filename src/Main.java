...
public class Main extends Application {
    ...
    private ImageView backgroundView; // Переместил сюда

    @Override
    public void start(Stage primaryStage) {
        Pane root = new Pane();

        // Определение пути к Background.jpg в той же директории, где находится JAR файл
        File jarFile = new File(Main.class.getProtectionDomain().getCodeSource().getLocation().getPath());
        File jarDir = jarFile.getParentFile();
        File imageFile = new File(jarDir, "/Background.jpg");
